...
from photos.resource.signal_bus import signalBus
...
class FaceAndGestureDetect:
    def __init__(self):
        # 设置摄像头画面大小
        WCam = 1280
        HCam = 640
        self.test=1
        signalBus.testSignal.emit()
    ...
    def run(self):
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                confidence_scores = self.model.decision_function([np.array(pre_processed_landmark_list)])
                ...
                if wait == 0 and predicted_character in ['0', '1', '2'] and len(faces) > 0:
                    for (x, y, w, h) in faces:
                        ...
                        if (predicted_character=='0') and (vo>=1):
                            ...
                            vo += 1
                        '''
                        cv2.putText(frame, f"Photo Count: {photo_count}, pose: {predicted_character}", (10, 70),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                        wait = 1
                        pre_time = time.time()
                        # volume.SetMasterVolumeLevel(vol_tansfer(vo), None)
            # cv2.imshow('PICS capture', frame)
            signalBus.updateMainCameraSignal.emit(frame)
            signalBus.testSignal.emit()
            key = cv2.waitKey(1)
            # 如果按下 'q' 键，退出循环
            if key == ord('q'):
